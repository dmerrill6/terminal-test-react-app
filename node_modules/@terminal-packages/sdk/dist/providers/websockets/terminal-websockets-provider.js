...
class TerminalWebsocketProvider extends terminal_base_provider_1.TerminalBaseProvider {
    constructor(options) {
        ...
        this.web3Version = enums_1.Web3Versions.two;
        if (options.web3Version) {
            this.web3Version = options.web3Version;
        }
        // @ts-ignore
        if (options.customWebsocketProvider) {
            ...
            if (this.web3Version === enums_1.Web3Versions.one) {
                this.host =
                    // @ts-ignore
                    customProviderOptions.customWebsocketProvider.connection.url;
            }
            else {
                this.host = customProviderOptions.customWebsocketProvider.host;
            }
            if (!this.host || this.host.length === 0) {
                throw new Error('Your custom websockets provider host is not defined or is set as a empty string.');
            }
        ...
        if (this.web3Version === enums_1.Web3Versions.two) {
            ...
            this.setChainId(new web3_providers_1.WebsocketProvider(this.host), enums_1.Web3Versions.one);
        }
    }
    /**
            ...
            if (this.web3Version === enums_1.Web3Versions.two) {
                ...
                if (this._lastNodeSavedResult) {
                    if (this._lastNodeSavedResult.nodeResult ===
                        this.parseJsonRPCResult(result.params.result)) {
                ...
                if (shouldSaveLogs) {
                    ...
                    this.saveToLogs(json_rpc_mapper_1.default.toPayload(result.method, []), result.params.result, undefined, event === 'error');
                }
            }
            callback(result);
        });
